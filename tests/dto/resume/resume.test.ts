...
import { resumeDataSchema, cvDataSchema, webResumeDataSchema } from '../../../dto/resume/resume-data'
...
describe('webResumeDataSchema', () => {
    ...
    test('필수 필드가 없으면 거부한다', () => {
        const result = webResumeDataSchema.safeParse({ profile: { firstName: 'Hyunseok' } })
        expect(result.success).toBe(false)
    })

    test('職務経歴書(cv) 구조를 거부한다', () => {
        const result = webResumeDataSchema.safeParse(validCvData)
        expect(result.success).toBe(false)
    })

    test('site 가 없는 프로젝트를 허용한다', () => {
        const { site: _site, ...projectWithoutSite } = validWebResumeData.workExperiences[0].projects[0]
        const result = webResumeDataSchema.safeParse({
            ...validWebResumeData,
            workExperiences: [{ ...validWebResumeData.workExperiences[0], projects: [projectWithoutSite] }],
        })
        expect(result.success).toBe(true)
    })
...
describe('resumeCreateSchema', () => {
    ...
    test('type=web + webResumeData 구조를 통과한다', () => {
        const result = resumeCreateSchema.safeParse({
            type: 'web',
            title: '웹 이력서',
            data: validWebResumeData,
        })
        expect(result.success).toBe(true)
    })
    ...
    test('type=cv인데 web 구조 data면 거부한다', () => {
        const result = resumeCreateSchema.safeParse({
            type: 'cv',
            title: '테스트',
            data: validWebResumeData,
    ...
    test('type=web인데 cv 구조 data면 거부한다', () => {
        const result = resumeCreateSchema.safeParse({
            type: 'web',
            title: '테스트',
            data: validCvData,
...
describe('resumeUpdateSchema', () => {
    ...
    test('web 구조 data 수정을 허용한다', () => {
        const result = resumeUpdateSchema.safeParse({ data: validWebResumeData })
        expect(result.success).toBe(true)
    })
})
...
describe('resumeListQuerySchema', () => {
    ...
    test('web type 필터를 적용한다', () => {
        const result = resumeListQuerySchema.safeParse({ type: 'web' })
        expect(result.success).toBe(true)
    })
