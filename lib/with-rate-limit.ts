...
export const withRateLimit =
    (deps: { checkLimit: CheckLimitFn }) => (handler: (c: Context, user: AuthUser) => Promise<Response>) => async (c: Context, user: AuthUser) => {
        const result = deps.checkLimit(user.id, c.req.path)
