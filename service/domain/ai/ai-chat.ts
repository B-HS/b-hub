...
export const createAiChatService = ({ connectionService, promptService, sessionService, attachmentService, logUsage }: AiChatDeps) => {
    const send = async (userId: string, sessionId: string, input: AiChatSend) => {
        ...
        if (input.attachmentIds?.length) await attachmentService.attachToMessage(userId, input.attachmentIds, userMessage.id)
        ...
        try {
            ...
            logUsage({
                ...
                severity: 40,
                errorDescription: error instanceof Error ? error.message.slice(0, 500) : 'unknown',
        ...
        await sessionService.touchLastMessage(sessionId)
        await connectionService.touchUsed(row.id)
    ...
    const complete = async (userId: string, input: AiCompletion) => {
        ...
        const { systemText, seedMessages } = assemblePrompts(prompts)
        const messages: AiChatMessage[] = [...seedMessages, ...input.messages.map((m) => ({ role: m.role, content: m.content }))]
