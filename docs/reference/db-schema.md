...
| 테이블 | 변경 | 도입 배치 | 주의 |
|--------|------|-----------|------|
| `mail_messages` | unique(`account_id`, `folder_id`, `remote_message_id`) — 제약명 `uq_mail_messages_account_remote` 유지 | 1차(D-05) | 2026-09-07 프로덕션 반영 완료 |
| `mail_messages` | index `idx_mail_messages_account_remote`(`account_id`, `remote_message_id`) | 2026-09-07 배포 후 수정 | 1차 unique 변경으로 사라진 Gmail 계정 단위 조회·삭제 경로 복원([bug](../bug/2026-09-07-mail-sync-account-remote-index.md)). **`db:push` 필요** |
