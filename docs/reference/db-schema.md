...
| 물리 테이블 | TS export | 핵심 컬럼 (총) | 인덱스·유니크 | FK·관계 | Drizzle 소유·사용 |
|------|------|------|------|------|------|
| `mail_accounts` | `mailAccounts` | `id`(PK), `user_id`, `provider`, `email`, `credentials`, `imap_*`/`smtp_*`, `is_active`, `last_sync_at`, `sync_cursor`, `better_auth_account_id`(소프트) (19) | uq(`user_id`,`email`); `idx_mail_accounts_user` | `user_id → user.id` (cascade) | `compose/mail.ts` (+ `scripts/backfill-thread-id.ts`) |
