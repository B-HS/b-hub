...
> 기준: 2026-09-07, b-hub `fix/audit-batch4-performance` @ 4차 배치 반영(1·2·3·4차 수정 배치 반영). 소비자 레포 지도와 전제는 [../acknowledge/2026-09-06-consumer-repos-and-compat.md](../acknowledge/2026-09-06-consumer-repos-and-compat.md), 수정 대상 목록은 [../quality-assurance/2026-09-06-audit-findings.md](../quality-assurance/2026-09-06-audit-findings.md).
>
> **4차 반영 요약**(성능 배치): **엔드포인트·상태 코드·성공 응답 본문은 어느 항목도 바뀌지 않았다.** 표면 변화는 응답 헤더 3곳뿐이다 — `GET /api/badge/image` 에 `CDN-Cache-Control`(기존 `Cache-Control` 과 같은 값) 추가, `GET /api/weather/locations` 에 `ETag`·`Cache-Control: private, max-age=3600` 추가 + `If-None-Match` 일치 시 **304**, `GET /api/calendar/:icsToken` 에 약한 `ETag` 추가 + `If-None-Match` 일치 시 **304**. 그 밖에 소비자가 관측할 수 있는 것: ① mail 메시지 upsert 가 건별 → 배치(identity SELECT → INSERT → 재조회 → UPDATE)로 바뀌었으나 `added`/`updated` 집계와 동기화 응답은 동일, ② 블로그 댓글·메시지 목록에 **동점 타이브레이크**(`commentId` desc·`id` desc)가 추가돼 같은 초에 저장된 행의 순서가 결정적으로 고정됨(동점이 아닌 행의 순서는 불변), ③ `api_token`·`metrics_token` 의 `lastUsedAt` 과 캘린더 구독 `lastAccessedAt` 이 **5분 단위**로만 갱신됨(어드민·manage 화면 표시 정밀도).
>
> **3차 반영 요약**(상세는 [../history/2026-09-07-audit-batch3.md](../history/2026-09-07-audit-batch3.md)): mail 동기화 중복 호출은 오류 대신 0 결과 성공 응답 · mail reply/forward 에 발송 rate limit(429·`X-RateLimit-*`) · AI 첨부 업로드 본문 413 · logs 수집 본문 413 · metrics 수집 본문 413 · calendar 이벤트 groupId 404·날짜 역전 400 · blog 댓글 404/403 · badge·spotify playing 공개 경로 429. **성공 응답의 본문 구조는 어느 항목도 바뀌지 않았다.**
...
| `POST /api/mail/messages/:messageId/reply` | `useReplyMail` `send/queries.ts:24-37` (`:28-31`); `widgets/reply/reply-section.tsx:80` | JSON `ReplyParams` `{ bodyHtml, to?, cc?, bcc?, attachmentIds? }` (`types-params.ts:44-50`) | nothing | `mailReplySchema` `dto:93-100`; `to` defaults to original sender (`mail-message.ts:453`); **3차 반영 — rate limited 20/min**, sharing the `send` bucket (`pathKey: 'mail:messages:send'`). Response now carries `X-RateLimit-Limit/Remaining/Reset`; over limit → 429 `RATE_LIMIT_EXCEEDED`. Consumer only toasts `error.message`, so this is compatible. | `MAIL_MESSAGE_NOT_FOUND`, `MAIL_SEND_FAILED`, `RATE_LIMIT_EXCEEDED` 429 |
| `POST /api/mail/messages/:messageId/forward` | `useForwardMail` `send/queries.ts:39-52` (`:43-46`); `reply-section.tsx:68` | JSON `ForwardParams` `{ to, cc?, bcc?, bodyHtml?, attachmentIds? }` (`types-params.ts:52-58`) | nothing | `mailForwardSchema` `dto:102-108`; server prepends forwarded header block (`mail-message.ts:483-484`); **3차 반영 — same shared 20/min bucket, same headers/429 as reply** | same + `RATE_LIMIT_EXCEEDED` 429 |
...
| `POST /api/mail/sync` | `useSyncIncremental` `queries.ts:27-43` (`:32-35`, `meta.suppressErrorToast`); callers `app/[locale]/mail/page.tsx:97-113,118-142` (all accounts in parallel via `Promise.allSettled`), `widgets/list/list.tsx:80` (pull-to-refresh), `widgets/account-settings/account-manager.tsx:27` | JSON `{ accountId, folderId? }` (`folderId` always `undefined` from page) | **`res.data.added`** (`page.tsx:104,129` → toast count). Type `SyncResult {added, updated, deleted, durationMs}` (`types-params.ts:3-8`) | `mailSyncTriggerSchema` `dto:3-6`; **rate limited** 20/min per user (`route:22-48`); returns `{ added, updated, deleted, durationMs }`; errors `MAIL_ACCOUNT_NOT_FOUND`, `MAIL_PROVIDER_ERROR` 502, `RATE_LIMIT_EXCEEDED`. Any failure → consumer toast `syncFailed` (`page.tsx:106,140`). **Client aborts after 30 s** (`shared/lib/api.ts:36`). **3차 반영 — account-level mutual exclusion**: if a sync for the same account is already running (`mail_accounts.last_sync_status = 'running'`, stale after 5 min), the request returns **200 with `{ added: 0, updated: 0, deleted: 0, durationMs }`** instead of erroring — chosen over 409 precisely because this consumer overlaps autosync with manual sync and would otherwise show `syncFailed` toasts. Consumers that read `data.added` for the toast count will see `0`. While a sync is in flight, `GET /api/mail/accounts` reports `lastSyncStatus: 'running'`, which this consumer compares only against `'error'`/`'success'` → neutral badge. **4차 반영(P-09·P-12·P-13·P-05)**: 메시지 upsert 가 건별에서 **배치**(identity 컬럼 SELECT `inArray` 1회 → 신규 INSERT → 재조회 → 기존 UPDATE)로 바뀌고, 원격 플래그 반영은 계정당 provider 연결 1회 후 폴더를 순회하며, Gmail access token 은 만료 5분 전 선제 갱신한다(선제 갱신 실패는 삼키고 기존 401 후 갱신 경로로). `added`/`updated`/`deleted`/`durationMs` 집계 규칙과 응답 본문은 불변이다 |
...
| Method + path | Call site | Request | Response read | Server |
|---|---|---|---|---|
| `GET /api/ai/providers` | `useAiProviders` `queries.ts:19-27` (`:22`, `retry: false`); `widgets/header/mail-header.tsx:94-95`, `ai-chat-widget.tsx:53-55` | none | `data[]` → `AiProviderConnection {id, provider, status, displayName}` (`types.ts:7-12`); read `status === 'active'`, `provider` (must be `'codex'|'anthropic'|'ollama'` `types.ts:1-3`), `displayName` (rendered) | `route/ai/connection.ts:34-47` → `toResponse` (`:17-29`): `{ id (number), provider, authType, status, statusDetail, displayName (**string**, `row.displayName ?? row.provider` 폴백 — 2차 배치 C-04), lastUsedAt, lastRefreshedAt, modelsFetchedAt, createdAt, updatedAt }`. Consumer types `id: string` (unused). |
| `GET /api/ai/:provider/models` | `useAiModels` `queries.ts:29-38` (`:32`, `retry:false`, enabled when provider) | path `provider` | `data[]` → `AiModel {modelId, displayName}` (`types.ts:14-17`); `modelId` used as `models[0]?.modelId` default (`ai-chat-widget.tsx:59`), `displayName` rendered; `isSuccess && length === 0` → "empty" UI (`:73`) | `route/ai/model.ts:34-48` → `{ id, providerId, modelId, displayName (**string**, `row.displayName ?? row.modelId` 폴백 — 2차 배치 C-04), metadata, fetchedAt }` (`:16-23`); provider name parsed by `aiProviderNameSchema` else `AI_PROVIDER_NOT_FOUND`; `resolveClient` may throw `AI_REAUTH_REQUIRED` 401 |
...
| `POST /api/calendar/events/create` | `createEventAction` `api.ts:71-92`; `useCreateEvent` `query.ts:48-58` | JSON `{ title, startDate, endDate (all-day: +1 day `:11,76`), startTime, endTime, isAllDay, groupId (`'' → null` `:80`), description, location, status: UPPERCASE (`:83`), color }` (undefined keys dropped by JSON) ; client pre-validates `eventFormSchema` (`validate.ts:5-40`: title ≤100, description ≤500, location ≤200) | `res.success` else throw `Error(res.error.message)`; `toClientEvent(res.data)` | `route:144-157` `createEventBodySchema` (`mapper:5-27`: title ≤500, `groupId` nullable, `status` enum `CONFIRMED|TENTATIVE|CANCELLED`); responds **201** + envelope (consumer ignores status). **3차 반영**: `dtend < dtstart` → 400 `VALIDATION_ERROR`, `groupId` 가 본인 소유가 아니거나 없는 그룹이면 → 404 `CALENDAR_GROUP_NOT_FOUND`. Calendar 는 자기 그룹만 보내고 폼이 종료 ≥ 시작을 강제하므로 정상 경로는 불변 | `VALIDATION_ERROR` → toast `eventCreationFailed(message)` (`query.ts:53`), `CALENDAR_GROUP_NOT_FOUND` 404 |
| `PATCH /api/calendar/events/:uid` | `updateEventAction` `api.ts:94-110`; `useUpdateEvent` `query.ts:60-84` (optimistic merge + rollback) ; callers `calendar-widget.tsx:54-56,69-71` | JSON = `{ ...input }` spread (may include `id`, `title`, …) with `endDate` (+1 day **only if `input.isAllDay` is present and true** `:97-100`), `groupId` (`undefined` untouched, `'' → null` `:101`; test `tests/entities/calendar/api.test.ts:50-66`), `status` UPPERCASE; `uid` pre-validated by `uidSchema` (`validate.ts:56-63`, regex `^[A-Za-z0-9._@-]+$`) | `res.success` else throw; `toClientEvent(res.data)` | `route:207-224`: `patchEventBodySchema = createEventBodySchema.partial()` (`mapper:29`; unknown keys such as `id` are stripped), merge via `toEventPatch` (`mapper:80-127`), returns `toEventResponse`. **3차 반영**: `startDate` 와 `endDate` 를 **둘 다** 보낸 경우 병합 결과가 역전이면 400 `VALIDATION_ERROR`, 남의/없는 `groupId` 는 404 `CALENDAR_GROUP_NOT_FOUND`. 한쪽 날짜만 보내는 부분 수정은 검사 대상이 아니다 | `CALENDAR_EVENT_NOT_FOUND` 404, `CALENDAR_GROUP_NOT_FOUND` 404, `VALIDATION_ERROR` → toast `eventUpdateFailed(message)` + rollback |
...
| `PATCH /api/calendar/groups/:id` | `updateGroupAction` `api.ts:133-142`; `useUpdateGroup` `query.ts:123-146` (optimistic `visible`); callers `calendar-widget.tsx:62-66` (`{ isVisible }`), `:77-79` (`{ name?, color? }`) | `{ name?, color?, sortOrder?, isVisible? }` (client `groupUpdateSchema` `validate.ts:49-54`); `id` pre-validated by `uidSchema` | `res.success`; `toCalendarGroup(res.data)` | `updateGroupSchema` `dto:8-13`; `route:43-58` updates then returns `getGroupById`; **`null` 이면 `CALENDAR_GROUP_NOT_FOUND` 404 를 던진다**(`route/calendar/group.ts:55`) — `data: null` 이 나가지 않는다(2차 배치 C-10) | `CALENDAR_GROUP_NOT_FOUND` 404 → toast + rollback |
...
| `GET /api/calendar/:icsToken` (browser navigation) | `features/calendar/calendar-export.tsx:34-40`: `<a href={icsUrl} download='calendar.ics'>` | none (public token) | file download; cross-origin so the `download` attribute is ignored — **server `Content-Disposition: attachment` is load-bearing** | `route/calendar/ics.ts`: `text/calendar`, `Content-Disposition: attachment; filename="<name>.ics"`, `Cache-Control: no-cache, no-store, must-revalidate`; `CALENDAR_SUBSCRIPTION_NOT_FOUND`. **4차 반영(P-18)**: `DTSTAMP:` 줄을 뺀 본문 해시로 약한 `ETag`(`W/"<32자 hex>"`)를 추가하고, `If-None-Match` 에 그 값(또는 `*`)이 있으면 **본문 없이 304**(`ETag`·`Cache-Control` 만). Calendar 는 `<a download>` 브라우저 내비게이션이라 조건부 요청을 보내지 않으므로 200 경로가 그대로 쓰이고, 200 응답의 본문·헤더는 불변이다 |
...
|---|---|---|---|---|
| `GET /api/ai/providers` | `getAiProviders` `api.ts:33-41`; `useAiProviders` `query.ts:14-19` (`retry:false`); `calendar-widget.tsx:39-40` (`hasAi = some(status==='active')`), `ai-chat-widget.tsx:33,40-44` | header `Accept: application/json` | `!res.ok` → `AiChatError('AI_PROVIDERS_FAILED')`; body → `parseAiProviders` (`types.ts:26`): unwrap `{success:true,data}` if present (`:19-24`) then **strict `z.array(z.object({ provider: enum['codex','anthropic','ollama'], status: enum['active','reauth_required','disabled'], displayName: z.string() }))`** (`:6-10`) — any extra value or `null` `displayName` throws | `route/ai/connection.ts:34-47`; `displayName` 은 **항상 `string`** — DB 컬럼은 nullable 이지만 응답 매핑이 `row.displayName ?? row.provider` 로 폴백하고 DTO 도 `z.string()`(`dto/ai/provider.ts:48`). 소비자의 strict 스키마와 정합(2차 배치 C-04) |
| `GET /api/ai/:provider/models` | `getAiModels` `api.ts:43-51`; `useAiModels` `query.ts:21-26` (`skipToken` when no provider) | `Accept: application/json` | `!res.ok` → `AI_MODELS_FAILED`; `parseAiModels` (`types.ts:28`): strict `{ modelId: string, displayName: z.string() }` (`:13-17`) | `route/ai/model.ts:34-48`; `displayName` 은 **항상 `string`** — 프로바이더가 `?? null` 로 채워도(`providers/anthropic-provider.ts:109`, `codex-provider.ts:120`) 응답 매핑이 `row.displayName ?? row.modelId` 로 폴백하고 DTO 도 `z.string()`(`dto/ai/model.ts:7`). 2차 배치 C-04 |
...
3. **[HIGH→해소(2차)][Calendar] Strict AI zod schemas vs nullable/extensible server fields.** `aiProviderSchema.displayName: z.string()` and `aiModelSchema.displayName: z.string()` (`Calendar/entities/ai/types.ts:9,15`). `displayName` 은 **2차 배치에서 `provider`/`modelId` 폴백으로 항상 문자열이 되어 해소됐다**(`route/ai/connection.ts:23`, `route/ai/model.ts:20`). 이전 서술: b-hub returned `displayName: string | null` (`dto/ai/provider.ts:48`, `dto/ai/model.ts:7`, `service/domain/ai/ai-connection.ts:95`, `providers/anthropic-provider.ts:109`, `providers/codex-provider.ts:120`). A single `null` makes the whole array parse throw → AI toggle hidden / model list empty. Same for the `provider`/`status` enums: adding a new provider or status value on the server breaks the Calendar AI panel entirely.
...
3. **[H→해소(2차)] 썸네일 조회수 부작용** — `GET /:id/thumbnail` 이 이제 `getByIdWithoutView` 를 쓴다(`HUB/route/blog/thumbnail.ts:44`, `service/domain/blog/post.ts:88`) → **OG 요청이 조회수를 올리지 않는다**. bblog 는 `views` 를 어디서도 렌더하지 않아 비호환 없음. 단 `Content-Type: image/png` + `Cache-Control: public, max-age=2592000, immutable` (182-187) 은 크롤러 계약 — 유지.
...
6. **[M→해소(2차)] complete `width/height` 0 → 400** — upload-server `getMetadata` 가 크기를 못 읽으면 `0` 을 보낸다(`UP/index.ts:39-42`). hub 스키마가 `nonnegative().nullable().transform(0 → null)` 로 바뀌어(`HUB/dto/blog/image.ts:16-27`) **`0` 을 받아 `null` 로 저장**한다. 업로드 전체가 400 으로 실패하던 경로가 사라졌고, upload-server 는 무수정이다.
...
9. **[M→적용(2차, 승인 A-5)] `GET /api/blog/posts/:id` 공개 필터** — 이제 비admin 요청은 미발행·숨김 글에 **404**(`HUB/route/blog/post.ts:56-58`, `service/domain/blog/post.ts:80-86`), 목록도 비admin 이면 `isPublished=true`·`isHide=false` 강제(`route/blog/post.ts:29-31`). bblog 편집 페이지(`BB/app/(editor)/edit/[id]/page.tsx:19`)는 **admin 세션 쿠키를 전달하므로 초안 로드가 유지**된다. 비로그인 상태의 초안 URL 직접 열람만 막힌다. 예외: `/:id/thumbnail` 에는 공개 필터가 없다.
...
| 6. complete | upload-server → b-hub `POST /api/drive/assets/:assetId/complete` | JSON `{ uploadToken, fileHash(sha256 재계산), storageTiers: 'L1,L3' 등 정렬 CSV(빈 문자열 가능), gdriveFileId \| null, localPath \| null, thumbnailBase64 \| null, **sizeBytes**(디스크 실측 `statSync(tmpPath).size` — 2차 배치 추가) }` + Bearer secret | `!ok` → 업로드 실패 처리; 본문 미사용 | `upload-handler.ts`; b-hub `route/drive/asset.ts:86-119`(`requireUploadServer` 게이트 `:104`) → `drive-asset.ts` `complete`: 토큰 불일치 `UNAUTHORIZED`, 상태 preparing/uploading 아니면 `DRIVE_UPLOAD_EVENT_FAILED`, **`sizeBytes` 가 prepare 신고값과 다르면 쿼터 재계산 → 초과 시 `failed` 마감 + 업로드된 티어 실물 정리 후 `DRIVE_QUOTA_EXCEEDED`(413)**, **`fileHash` 갱신이 `uq_cloud_assets_user_hash` 를 위반하면 같은 정리 후 `DRIVE_DUPLICATE_FILE`(409, 이전 500)**, `storageTiers` 비면 `uploadStatus='failed'` 아니면 `'ready'`, `uploadToken` null 화, 응답 `{ id, uploadStatus }` |
...
|-------------|-------------|------|--------------------------|------------|------|
| `GET /api/ai/providers` | `src/entities/ai/api.ts:23-32`; 게이트 `src/widgets/ai-chat/ai-chat-panel.tsx:7-11`(`status === 'active'` 만) | — | `aiProviderListSchema = [{ provider: 'codex'\|'anthropic'\|'ollama', status: 'active'\|'reauth_required'\|'disabled', displayName: z.string() }]` (`schema.ts:4-10`, `constants.ts:1-6`) — 파싱 실패 시 쿼리 에러 → 패널 숨김 | `[{ id, provider, authType, status, statusDetail, displayName: **string**(항상), lastUsedAt, lastRefreshedAt, modelsFetchedAt, createdAt, updatedAt }]` (`route/ai/connection.ts:17-29,34-47`; DB 는 여전히 nullable — `service/domain/ai/ai-connection.ts:95`, `/manage` 빈 입력 `page/manage/pages/ai.tsx:158` — 이지만 응답 매핑이 `?? row.provider` 로 폴백, 2차 배치 C-04) | UI 표시 `info.displayName` (`ai-chat-sheet.tsx:139`) → M19 해소 |
| `GET /api/ai/:provider/models` | `api.ts:34-43`; `ai-chat-sheet.tsx:46,50-51` | — (패널 열릴 때만) | `[{ modelId: z.string(), displayName: z.string() }]` (`schema.ts:12-17`) | `[{ id, providerId, modelId, displayName: **string**(항상, `?? row.modelId` 폴백), metadata, fetchedAt }]` (`route/ai/model.ts:16-23,34-48`); 프로바이더 문자열 무효 → `AI_PROVIDER_NOT_FOUND`(404), 연결 없음/disabled → 404, reauth → `AI_REAUTH_REQUIRED`(401) (`ai-connection.ts:113-118`) | anthropic 모델은 `display_name ?? null` (`providers/anthropic-provider.ts:109`) 이지만 응답 매핑에서 `modelId` 로 폴백 → M19 해소 |
...
| `/api/ai/sessions*`, `/api/ai/prompts*`, `/api/ai/attachments*`, `/api/ai/completions`(비스트림) | **사용 없음** | | | **3차 반영(미소비 경로)**: `POST /api/ai/attachments` 에 본문 21MB `bodyLimit` 이 붙어 초과 시 413 `AI_ATTACHMENT_TOO_LARGE`(기존 서비스 단 20MB 검증과 같은 코드) | |
...
- **M7 [M→해소(1차)] 세 콜백 모두 secret 검사**: `/status`·`/complete`·`/gdrive-token` 전부 `requireUploadServer` 게이트다(`route/drive/asset.ts:78`·`:104`·`:132`, `UPLOAD_SERVER_SECRET` 미설정 시 `SERVICE_NOT_CONFIGURED` 503). upload-server 는 세 호출 모두 `Authorization: Bearer <secret>` 을 보내므로 호환이고, `/status` 는 fire-and-forget 이 아니라 업로드 진행 게이트가 됐다(§1.5 3단계). `uploadToken` 검증 순서(토큰→상태)와 에러 코드(`UNAUTHORIZED`/`DRIVE_UPLOAD_EVENT_FAILED`/`VALIDATION_ERROR`)는 upload-server 가 분기하지 않으므로 상태 코드만 2xx/비2xx 로 유지하면 된다.
- **M8 [L→해소(2차)] `prepare` 본문 Zod 검증.** `route/drive/asset.ts:63` 에 `validator('json', driveAssetPrepareSchema)` 가 붙었다(`dto/drive/asset.ts:16-22`). Storage 는 5개 필드를 항상 보내므로(`upload.ts:54-60`) 정상 경로는 그대로다. 검증 실패는 400 이며 Storage 는 `!res.ok || !json.success` → throw 라 동작 동일. 부수 효과로 `sizeBytes` 가 `NaN`·`0` 으로 흘러들어 쿼터 계산이 무너지는 경로가 막혔다.
- **M9 [M] 전 계층 실패 시퀀스.** upload-server 는 `storageTiers=''` 로 `complete` 를 먼저 호출(b-hub 가 `failed` 로 마킹)한 뒤 500 을 돌려준다(`upload-handler.ts`). 2차 배치 이후 `complete` 자체가 쿼터 초과(413)·해시 중복(409)으로 throw 할 수 있으며, 그 경우 b-hub 가 자산을 `failed` 로 마감하고 이미 올라간 L1/L3 실물을 정리한다 — upload-server 는 `!ok` 만 보므로 동작은 기존 실패 경로와 같다. Storage 는 실패 toast 후 목록 무효화(`storage-layout.tsx:234-238`) → "업로드 실패" 행이 최대 10분 보임(`compose/drive.ts:149-153`). 스테일 필터 시간을 바꾸면 UI 노출 시간이 바뀐다.
...
- **M19 [H→해소(2차)] AI `displayName` null.** 소비자 스키마는 `z.string()`(`src/entities/ai/schema.ts:7,14`) 인데 b-hub 응답이 `null` 이면 Zod 실패 → providers 쿼리 에러 → **AI 패널 전체 숨김**(모델이면 목록 0개) 이었다. 2차 배치에서 응답 매핑이 `row.displayName ?? row.provider`(`route/ai/connection.ts:23`)·`row.displayName ?? row.modelId`(`route/ai/model.ts:20`)로 폴백하고 DTO 도 `z.string()` 이 되어 **소비자 무수정으로 해소**됐다. DB 컬럼(`ai_providers.display_name`)과 프로바이더 원본(`providers/anthropic-provider.ts:109`)은 여전히 nullable 이므로, 새 응답 경로를 추가할 때 폴백을 빠뜨리지 않는다.
...
- **M32 [L]** b-hub 런타임 의존 없음. `badge.hyns.dev/api/image` 는 별도 서비스. b-hub 로 이전 시에만 `/api/image` 리라이트와 `dto/badge.ts` 파라미터 의미 보존이 계약.
- **M33 [L] 3차 반영 — `GET /api/badge/image` 공개 제한 2종.** ① IP(`x-forwarded-for` 첫 값 → `x-real-ip`) 기준 **분당 60회**, 초과 시 429 `RATE_LIMIT_EXCEEDED`(200 PNG 응답에도 `X-RateLimit-Limit/Remaining/Reset` 헤더가 붙는다). ② `width * height > 2,000,000` 이면 400(개별 상한 4096 은 유지). 포트폴리오가 쓰는 배지 크기(수백 × 수십 px)는 상한 안이고, `next/image unoptimized` 렌더는 페이지당 요청 수가 적어 분당 60회에 닿지 않는다. b-hub 로 이전한다면 이 두 제한이 새 계약 표면이다.
- **M34 [L] 3차 반영 — `GET /api/spotify/playing/:token`(+`/widget`·`/data`) 도 공개 rate limit 대상.** 키는 `토큰:IP`, 분당 60회, 같은 3개 헤더 + 429. 위젯 HTML 의 기본 폴링(5초)은 분당 12회라 여유가 있으나, 한 IP 뒤에서 같은 토큰으로 다수가 보면 합산된다. 계약 문서상 런타임 소비자는 확인되지 않았다.
...
| Quota | rolling 24 h `COUNT(*)` of `weather_api_log` rows for the key `< dailyLimit`; default `dailyLimit = 100`. **4차 반영(P-17)**: 그 `COUNT(*)` 를 키 검증 SELECT 의 상관 서브쿼리로 합쳐 왕복 1회로 줄이고, 값을 프로세스 내 5초 TTL 핸드오프(최대 100건, 1회 소비)로 `checkRateLimit` 에 넘긴다. 판정식·429 응답·`dailyLimit` 의미는 불변 | `service/domain/weather/weather-api-key.ts`, `db/schema.ts` |
...
**4차 반영(P-17) — `GET /api/weather/locations` 전체 목록에만**: 응답에 `ETag`(본문 sha256 앞 32자를 큰따옴표로 감싼 강한 검증자, 프로세스당 1회 계산)와 `Cache-Control: private, max-age=3600` 이 붙고, 요청 `If-None-Match` 가 그 값과 **문자열 완전 일치**하면 본문 없이 **304** 를 반환한다. 200 본문은 불변. 이 엔드포인트는 웹 클라이언트·펌웨어 어느 쪽도 호출하지 않으며(§2.3 "Unused by the web client", §3 은 `/locations/{keyword}` 만 호출), 조건부 헤더를 보내지 않는 클라이언트는 종전대로 200 만 받는다. `/locations/:keyword`·`/locations/convert` 는 변경 없다.
...
- Cache: Redis + 30 s in-memory; TTL to next KMA boundary — **4차 반영(P-17)**: 그 경계가 base time 산출과 같은 상수를 쓰도록 정렬돼 ncst 는 `:40`(이전 `:10`), fcst `:45`, vilage 는 다음 발표시각 +10 min 이다. min 30 s; **successes + NO_DATA (`resultCode='03'`, 30 s negative cache) are cached** — 그 밖의 실패는 캐시하지 않는다 (`kma-api.ts`, `service/shared/redis-cache.ts:24,33`). Consumers polling every 10 min will mostly hit cache.
...
| Item | Value | Evidence |
|---|---|---|
| `POST /api/logs` | header `X-Device-Key`; body = one event; 200 `{ success: true, data: { id } }`. **3차 반영**: request body over **1 MB** → 413 `LOG_BATCH_TOO_LARGE` (`bodyLimit` before validation) | `route/logs/log-event.ts` |
| `POST /api/logs/batch` | `{ events: [1..50] }`; empty array → 400 (schema `min(1)`); `> 50` → 413 `LOG_BATCH_TOO_LARGE`; 200 `{ success: true, data: { count } }`. **3차 반영**: same 1 MB body cap, same error code | `route/logs/log-event.ts`, `dto/logs/log-event.ts:26-28` |
| `GET /api/logs/purge` | **3차 신설** — Vercel Cron only (`40 4 * * *`), `verifyCronAuth` (`Authorization: Bearer <UPLOAD_SERVER_SECRET>` or `x-cron-secret`). GET-only so the existing admin `POST /api/logs/purge` is untouched. 200 `{ success: true, data: { infoDeleted, warnDeleted, errorDeleted, weatherApiLogDeleted, mailSyncLogDeleted, mailSyncSessionDeleted } }` | `route/logs/purge.ts`, `vercel.json` |
...
- 서버측 검사 순서(3차 반영): 토큰(401) → rate limit(429) → **본문 크기 `bodyLimit`(413)** → 바디 Zod(400) → payload 크기(413) (`B/route/metrics/ingest.ts`, `require-metrics-token.ts:18-29`).
...
- 클라이언트: `serde_json::to_vec(payload).len() > 65536` **바이트**(`sender.rs:45-46`). 서버도 **바이트 기준**으로 통일됐다 — `Buffer.byteLength(JSON.stringify(payload)) > 65536`(`B/route/metrics/ingest.ts:23`, `B/dto/metrics/ingest.ts:3`, 2차 배치 E-23). 이전의 UTF-16 코드유닛 비교는 멀티바이트 payload 에서 상한을 사실상 무력화했다. 서버는 파싱 후 재직렬화 기준이라 실수 `95.0`→`95` 처럼 더 짧아질 수 있어 여전히 클라이언트 검사가 같거나 더 엄격 → 불일치 없음.
- 고정 규칙: `METRICS_PAYLOAD_MAX_BYTES` 를 65536 미만으로 내리거나 `>=` 로 바꾸면 클라이언트가 통과시킨 이벤트가 413 이 된다(단건이면 drop, 배치면 무의미한 분할 반복).
- **3차 반영 — 요청 본문 상한이 추가됐다**: `POST /api/metrics/ingest` 는 본문 **128 KB**, `/ingest/batch` 는 **4 MB** 를 넘으면 DTO 파싱 전에 413(`METRICS_PAYLOAD_TOO_LARGE` / `METRICS_BATCH_TOO_LARGE`, `errorResponse` 봉투)이다. 정상 클라이언트는 payload 64 KB × 최대 50건이라 두 상한 모두 여유가 있다(50 × 64 KB ≈ 3.2 MB < 4 MB). **payload 상한을 올릴 경우 이 본문 상한도 함께 올려야** 배치가 413 에 걸리지 않는다.
...
### 5-9. series 배열 인덱스 dot-path — 서버에서 지원 (2차 배치 C-11)
- 클라이언트는 `cpu.cores.0.usage` 같은 **배열 인덱스 경로**를 후보로 제시하고(`payload.ts:4-6`, `device-detail.tsx:37`), 서버 regex 도 숫자 세그먼트를 허용한다(`B/dto/metrics/query.ts:14-18`).
- 서버 `$project` 가 문자열 경로 대신 `buildSeriesValueExpression`(`B/compose/metrics.ts:18-35`)이 만든 표현식을 쓴다. 숫자 세그먼트를 만나면 `$let` + `$cond[$isArray]` 로 갈라 **배열이면 `$arrayElemAt`, 객체면 숫자 키 접근**으로 값을 뽑고 나머지 경로를 재귀 처리한다. 이전의 `'$payload.cpu.cores.0.usage'` 문자열 경로는 배열 인덱스를 해석하지 못해 `v` 가 missing 으로 나가고, Rust `SeriesPoint.v: f64` 역직렬화 실패(`api.rs:65-69`) → series 호출 전체 실패 → UI "선택한 기간에 데이터가 없습니다"(`device-detail.tsx:113-114`) 로 이어질 수 있었다.
- `$match` 는 여전히 `payload.<field>` 문자열 경로다(Mongo dot-notation 이 배열을 암묵 순회하므로 필터는 정상). 고정 규칙: `{t, v}` 필드명과 `v` 가 항상 숫자로 나가는 계약은 유지한다.
