...
| `calendar_subscription` | `id`(PK), `user_id`, `token`(unique), `ics_token`(unique), `name`, `is_active`(기본 true), `ctag`(기본 `'0'`), `last_accessed_at` | **`unique(user_id)` = `uq_calendar_subscription_user`**(`db:push` 필요). 4차에서 중복 인덱스 2종(`idx_subscription_token`·`idx_subscription_ics_token`)을 제거했다 — 각각 `token`·`ics_token` 의 unique 제약이 같은 컬럼을 이미 인덱싱한다. `idx_subscription_user` 는 `uq_calendar_subscription_user` 와 겹치지만 `user_id` 가 FK 라 유지한다(drizzle push 가 unique 생성 전에 DROP INDEX 를 실행해 `ER_DROP_INDEX_FK` 로 실패함, 2026-09-07 실측). `user_id` → `user.id` (cascade) |
