...
2. `listRecentMessages`(최근 50) + 첨부 이미지(`resolveImages` base64) 로 messages 구성. 사용자 메시지 저장(첨부 연결).
3. `client.complete` 호출(시간 측정). 성공 시 assistant 메시지 저장(tokens·durationMs), `touchLastMessage`·`touchUsed`, `logUsage`(severity 20). 실패 시 `logUsage`(severity 40 → Discord 알림) 후 재-throw.
