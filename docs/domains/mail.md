...
| `mail_sync_logs` | `sync_type`, `status`, `folder_id`, `messages_added/updated/deleted`, `duration_ms`, `error_message`, `started_at`, `completed_at` | idx(`account_id`,`created_at`) = `idx_mail_sync_logs_account_created`(4차에서 단일 컬럼 `idx_mail_sync_logs_account` 를 대체 — 접두가 같아 `account_id` 단독 조회도 커버) | `account_id`→`mail_accounts`(cascade) |
