...
> 기준: 2026-09-07 (fix/audit-batch4-performance @ 4차 배치 커밋 완료, 비교 기준 `bab14e8`) 코드 검증. 다루는 코드: `dto/mail/*`, `route/mail/*`, `service/domain/mail/**`, `compose/mail.ts`, `lib/mail-utils.ts`, `lib/mail-thread.ts`, `lib/credential-crypto.ts`(자격증명 암복호화 공용 구현), `lib/rate-limit.ts`, `scripts/backfill-thread-id.ts`, `db/schema.ts`(mail_* 테이블)
...
| `mail_sync_logs` | `sync_type`, `status`, `folder_id`, `messages_added/updated/deleted`, `duration_ms`, `error_message`, `started_at`, `completed_at` | idx(`account_id`) = `idx_mail_sync_logs_account`(FK 인덱스, 유지) + idx(`account_id`,`created_at`) = `idx_mail_sync_logs_account_created`(4차 추가) | `account_id`→`mail_accounts`(cascade) |
...
- **Gmail access token 은 만료 5분 전이면 선제 갱신**한다(`gmail-provider.ts` 의 `TOKEN_REFRESH_MARGIN_MS`). `getOAuthToken`(`compose/mail.ts`)이 better-auth `account.access_token_expires_at` 을 함께 돌려주고, provider 의 `getToken()` 이 `refreshToken` 과 만료 시각이 모두 있을 때 남은 시간이 5분 이하면 401 을 기다리지 않고 `refreshAccessToken`(401 경로와 **같은 single-flight `refreshPromise`**)을 부른다. **선제 갱신이 실패하면 오류를 삼키고** 기존 토큰·refresh 토큰을 되돌린 뒤 그대로 호출을 진행한다 — 그 호출이 401 이면 종전의 401 후 갱신 경로로 떨어지므로 최종 오류 매핑은 같다. 만료 시각을 모르는 계정(값 없음)은 종전처럼 401 시에만 갱신한다. `disconnect()` 는 캐시한 만료 시각도 함께 비운다.
...
- **폴더 카운트는 단일 쿼리**로 구한다: `countsByFolder(folderId)`(`compose/mail.ts`)가 `COUNT(*)` 와 `SUM(CASE WHEN is_read = 0 THEN 1 ELSE 0 END)` 를 한 번에 읽어 `{ messageCount, unreadCount }` 를 돌려주고, 동기화·플래그·이동/삭제 경로가 모두 이것을 쓴다(이전에는 `countMessagesByFolder`·`countUnreadByFolder` 2쿼리를 `Promise.all` 로 던졌다). 저장되는 `mail_folders.message_count`/`unread_count` 값은 같다.
...
- **upsert 는 배치 단위**다(`mail-sync.ts` 의 `upsertMessagesFromProvider` → `compose/mail.ts` 의 `upsertMessages`). 배치 100건을 건별로 "SELECT → UPDATE/INSERT → 재조회" 하던 것을 아래 순서로 바꿨다.
    1. 배치의 `remoteMessageId` 전체를 `inArray` 로 **한 번에 SELECT**(identityScope 에 따라 계정 또는 계정+폴더 조건, `id` 오름차순)해 기존 행 id 맵을 만든다.
    2. 기존 행이 없는 것만 INSERT 한다(행마다 1문, 가변 필드 `onDuplicateKeyUpdate` 동반 — **다중행 INSERT 로는 합치지 않았다**).
    3. 새로 넣은 `remoteMessageId` 들을 다시 한 번에 SELECT 해 PK 를 회수한다. account 스코프에서 같은 식별자의 행이 둘 이상이면 **가장 작은 id 만 남기고 나머지를 한 번의 `inArray` DELETE 로 정리**한다(라벨 이동으로 생긴 중복 행 정리 — 종전과 같은 규칙).
    4. 기존 행은 가변 필드(`subject`·`bodyHtml`·`bodyText`·`snippet`·`isRead`·`isStarred`·`isDraft`·`hasAttachments`·`threadId`·`messageIdHeader`·`inReplyTo`·`referencesHeader`)만 PK 기준으로 UPDATE 한다. 폴더·원격 id 는 건드리지 않는다.
    - 반환값은 메시지 순서대로 `{ id, isNew }` 배열이며, `added`/`updated` 집계 의미는 그대로다. 예전 구현이 반환하던 "저장된 메시지 행 전체" 는 더 이상 읽지 않는다(재조회 제거).
    - 배치 중간에 이벤트루프를 양보하던 `yieldEventLoop`(10건마다 `setTimeout(0)`)는 제거됐다 — 쿼리 왕복 자체가 배치화되면서 필요 없어졌다.
- **첨부는 배치 다중행 upsert** 다: 메시지 upsert 로 확정된 PK 를 붙여 배치 전체의 첨부를 모은 뒤 `upsertAttachments` 가 **단일 다중행 INSERT … ON DUPLICATE KEY UPDATE**(`values(...)` 로 `filename`·`mime_type`·`size_bytes`·`content_id`·`is_inline` 갱신)로 넣는다. 이전처럼 첨부 1건마다 INSERT + 재조회 하지 않으며, 반환값도 쓰지 않는다.
...
- **원격 플래그 반영은 계정당 연결 1회**다(`syncFlagActionToProvider` 의 `groupByAccount`). 계정 단위로 `getProvider` → `connect()` 한 뒤 **그 연결 안에서 폴더 그룹을 순회**하고, 마지막에 `finally` 로 `disconnect()` 한다. 여러 폴더에 걸친 읽음/별표 반영이 폴더 수만큼 IMAP 로그인·로그아웃을 반복하던 것을 없앤다. 폴더 하나가 실패해도 `captureException` 후 같은 연결로 다음 폴더를 계속 처리하고(전체가 중단되지 않는다), 계정 단위 실패는 종전처럼 바깥 `catch` 가 보고한다. provider 에 넘기는 인자(`remoteMessageId` 목록 + 그 폴더의 `remote_folder_id`)와 DB 갱신 결과는 같다.
- **이동·삭제는 여전히 계정+폴더 단위 그룹핑**이다(`groupByAccountFolder`). IMAP 은 UID 조작 전에 해당 mailbox 를 열어야 하므로, 각 그룹의 `remote_folder_id` 를 provider 에 넘겨 `runInMailbox`(`getMailboxLock` → 작업 → `release`)로 실행한다. `deleteMessage` 도 폴더 인자를 받아 같은 방식으로 `\\Deleted` 플래그 + `messageDelete` 를 UID 범위 한 번에 수행한다.
...
- **4차 성능 배치에서 일부러 남긴 것들**: ① 발신자 목록(`getSenderList`)은 여전히 메시지 행을 읽어 앱에서 주소→이름 맵으로 접는다 — `GROUP BY` 로 바꾸면 "상위 N" 의 의미(대상 행 집합)가 달라져서 미적용. ② 메시지 INSERT 를 다중행 한 문장으로 합치는 것은 account 스코프 중복 정리·PK 회수 규칙과 얽혀 미적용(첨부만 다중행). ③ IMAP/Gmail **프로토콜 호출 자체를 줄이는 항목(P-10·P-11)은 사용자 결정 대기**로 손대지 않았다. ④ 발송 경로의 `provider.connect()`(SMTP 전 IMAP 로그인)도 자격 검증 경로가 바뀌므로 그대로 둔다. `mark-all-read` 는 이미 단일 UPDATE 라 변경 없음.
