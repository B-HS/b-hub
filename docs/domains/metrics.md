...
> 기준: 2026-07-22 (dev @ `98f7492`) 코드 검증. 다루는 코드: `route/metrics/ingest.ts`, `route/metrics/token.ts`, `route/metrics/query.ts`, `route/metrics/heartbeat.ts`, `service/domain/metrics/token.ts`, `service/domain/metrics/log.ts`, `dto/metrics/token.ts`, `dto/metrics/ingest.ts`, `dto/metrics/query.ts`, `compose/metrics.ts`, `compose/types.ts`, `compose/index.ts`, `middleware/require-metrics-token.ts`, `lib/cron-auth.ts`, `db/schema.ts`(`metrics_token`), `db/mongo.ts`, `route/index.ts`, `index.ts`, `vercel.json`, `page/admin/pages/metrics.tsx`, `lib/error-code.ts`, `lib/error-message.ts`, `lib/error.ts`

## 개요

- 시스템 모니터링 대시보드(클라이언트 프로젝트명 **machboard**)의 수집 API 도메인. 클라이언트(Tauri 데스크톱/headless 데몬/ESP32)가 시스템 정보 JSON 을 주기 전송하면 저장·조회·시계열·하트비트 감지를 제공한다.
...
- **MongoDB**(비-Drizzle, `db/mongo.ts`, DB `metrics`):
  ...
  - `metrics_devices`(`MetricsDeviceDoc`): `deviceId`(unique)·`tokenId`·`tokenAlias`·메타(nullable)·`intervalSec`(nullable)·`firstSeenAt`·`lastSeenAt`·`downAlertedAt`(nullable). 인덱스 `{deviceId}` unique.
...
| `route/metrics/heartbeat.ts` | HTTP 경계 — `createMetricsHeartbeatRoute`. GET+POST `/heartbeat-check`, `verifyCronAuth`(cron 인증). `checkHeartbeats` 호출 → `{ down, recovered }` |
...
| `service/domain/metrics/log.ts` | 도메인 로직 — `createMetricsLogService`(ingest·list·listDevices·getDevice·series·checkHeartbeats), `MetricsLogServiceDb`·`MetricsAlerter`. online 판정 = `now - lastSeenAt < max(3×intervalSec, 5분)`. ingest 는 `receivedAt` 서버시각 + 디바이스별 최신 이벤트로 upsert |
...
| `compose/metrics.ts` | DI — `composeMetrics`가 `MONGODB_URI` 없으면 `{}` 반환. `tokenDb` 는 Drizzle(단 `countEventsSince` 는 Mongo 카운트), `logDb` 는 Mongo 구현. `DISCORD_WEBHOOK_URL` 있으면 60초 throttle `alerter` 주입 |
...
| `lib/cron-auth.ts` | `verifyCronAuth(c, secret)` — Bearer/`x-cron-secret` == secret 검증(실패 시 `UNAUTHORIZED`). heartbeat cron + drive lifecycle cron 공용 |
| `route/index.ts` | `/metrics/ingest`·`/metrics/tokens`·`/metrics`(query)·`/metrics`(heartbeat)로 마운트(`stub` 래핑, 더 구체적인 접두사 먼저) |
...
`route/index.ts`에서 `/metrics/ingest`·`/metrics/tokens`·`/metrics`로 마운트되고, `index.ts`에서 라우터 전체가 `/api`로 마운트된다. 인증 표기 `metrics-token(client|admin)` = `requireMetricsToken`(Bearer/`X-Metrics-Token`), `cron-secret` = `verifyCronAuth`.
...
| GET·POST | `/api/metrics/heartbeat-check` | cron-secret | 디바이스 다운/복구 감지. Vercel cron(`*/10 * * * *`). 응답 `{ down, recovered }` |
...
- **하트비트 감지·알림**: Vercel cron 이 10분마다 `/api/metrics/heartbeat-check`(cron 인증) 호출 → `checkHeartbeats` 가 디바이스 전수 순회. **다운 전이**(offline & `downAlertedAt` 없음) 시 `alerter`(severity 40, `METRICS_DEVICE_DOWN`) + `downAlertedAt` 마킹, **복구 전이**(online & `downAlertedAt` 있음) 시 `alerter`(severity 20, `METRICS_DEVICE_RECOVERED`) + 마킹 해제. `alerter` 는 `DISCORD_WEBHOOK_URL` 설정 시에만 활성이며 `errorCode:deviceId` 키로 60초 throttle(Discord 웹훅).
...
- **heartbeat cron 인증 시크릿은 `UPLOAD_SERVER_SECRET`**: `route/index.ts` 가 `cronSecret: deps.uploadServerSecret ?? ''` 로 주입한다(drive lifecycle cron 과 동일 시크릿·동일 `verifyCronAuth`). 미설정이면 빈 문자열이라 임의 호출이 통과할 수 있으므로 운영에서 반드시 설정한다.

## 관련 문서

- [../metrics-client-contract.md](../metrics-client-contract.md) — 클라이언트(machboard 에이전트·ESP32) 수집 계약(헤더·필드·제약·재시도·하트비트 의미) 정본.
