...
| 파일 | 역할 |
|---|---|
| `route/metrics/ingest.ts` | HTTP 경계 — `createMetricsIngestRoute`. 단건 POST `/` + 배치 POST `/batch`(둘 다 `requireMetricsToken` client scope + rate limit + `bodyLimit`). **본문 상한**: 단건 128KB → 413 `METRICS_PAYLOAD_TOO_LARGE`, 배치 4MB → 413 `METRICS_BATCH_TOO_LARGE`. payload 직렬화 **바이트 길이**(`Buffer.byteLength`) 64KB 초과 413, 배치 50건 초과 413 |
...
| Method | Path | 인증 | 설명 |
|--------|------|------|------|
| POST | `/api/metrics/ingest` | metrics-token(client)+rate limit | 단건 수집. **본문 128KB 초과** 또는 payload 직렬화 바이트 길이 64KB 초과 시 413 `METRICS_PAYLOAD_TOO_LARGE`. 응답 `{ count }` |
| POST | `/api/metrics/ingest/batch` | metrics-token(client)+rate limit | 배치 수집(events 1~50, 50 초과 413 `METRICS_BATCH_TOO_LARGE`). **본문 4MB 초과 시에도 같은 코드**. 응답 `{ count }` |
...
- **실행당 처리량은 제한된다**: 한 번의 호출에서 **최대 3일치**(`ARCHIVE_MAX_DAYS_PER_RUN`)만 처리하고, 각 일자 시작 전에 경과 시간이 **200초**(`ARCHIVE_TIME_BUDGET_MS`)를 넘었으면 중단한다. 밀린 일자는 다음 실행에서 이어 처리된다 — 하루치 전체를 메모리에 모아 동기 gzip 하는 작업이 서버리스 실행 시간을 넘겨 통째로 실패하던 위험을 줄인다.
