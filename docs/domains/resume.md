...
- 한 `resumes` 테이블에 두 종류(`type`)의 문서를 JSON(`data`) 컬럼으로 보관한다: `resume`(일본 履歴書 양식), `cv`(職務経歴書 = 경력기술서 양식). 구분은 `type` 컬럼과 생성 시 discriminated union 으로 한다.
...
| 파일 | 역할 |
|---|---|
| `dto/resume/resume-data.ts` | 이력서 본문 데이터 Zod 스키마 — `resumeDataSchema`(履歴書), `cvDataSchema`(CV) + 하위 스키마(contact/history/experience/overview/job). 타입 `ResumeData`·`CvData` export |
...
모든 스칼라 필드는 문자열 스키마다(`gender`/`marital_status`/`spouse_obligation`은 enum 으로 제약된 문자열). 배열 필드(`history`/`qualifications`/`overview`/`jobs`)는 전부 객체 배열이며, 문자열 배열이나 숫자 필드는 없다. 날짜·나이·통근시간도 전부 문자열이다.

## API 엔드포인트

마운트: `index.ts`가 `app.route('/api', api)`, `route/index.ts`가 `router.route('/resume', ...)` → 실제 경로는 `/api/resume*`. 모든 엔드포인트가 세션 인증 필요(`getSession` 없으면 `UNAUTHORIZED`).
...
| 파일 | 커버리지 |
|---|---|
| `tests/dto/resume/resume.test.ts` | 스키마 — resume/cv data 파싱, discriminated union 교차 거부(type↔data 불일치), `isPublic` 기본값 false, list 쿼리 기본값·type 필터 |
| `tests/service/domain/resume/resume.test.ts` | 서비스 — list, getById(자신/not_found/not_owner), create, update·delete 소유권 |
| `tests/route/resume/resume.test.ts` | 라우트 — 5개 엔드포인트, 미인증 401, 미존재·타인 리소스 404 |
| `tests/page/admin/resumes.test.ts` | 어드민 — 목록(공개 뱃지/검색 prefill/size clamp 5~100/page 폴백), 상세 data JSON, id 0 처리, 공개토글 |

## 주의사항 / 함정

- **slug 없음, 공개 read 엔드포인트 없음**: 접근은 숫자 `id` + 소유자 세션뿐이다. `is_public`은 저장·어드민 토글만 되고, 이를 게이트로 삼아 비인증/타인에게 이력서를 서빙하는 라우트는 현재 코드에 없다(전수 grep 확인). 즉 `is_public`은 아직 read 소비처가 없는 저장 플래그다.
