...
| [metrics-client-contract.md](./metrics-client-contract.md) | machboard 에이전트·ESP32 가 지켜야 할 시스템 정보 수집 계약(헤더·필드·재시도·온라인 판정) |
...
| [domains/metrics.md](./domains/metrics.md) | 시스템 모니터링(machboard) 수집 — 토큰(MySQL)·로그/디바이스(MongoDB) 하이브리드, scope(client/admin), 조회 시 온라인 판정 |
