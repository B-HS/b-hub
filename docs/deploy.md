...
| 대상 | 변경 | 도입 | 주의 |
|------|------|------|------|
| `mail_messages` | unique `(account_id, folder_id, remote_message_id)`(제약명 `uq_mail_messages_account_remote` 유지) | 1차 배치(D-05) | 2026-09-07 반영 완료 |
| `mail_messages` | index `idx_mail_messages_account_remote (account_id, remote_message_id)` | 2026-09-07 배포 후 수정 | Gmail 계정 단위 조회·삭제가 전체 스캔·lock wait 로 실패하던 문제([bug/2026-09-07-mail-sync-account-remote-index.md](./bug/2026-09-07-mail-sync-account-remote-index.md)). **적용 전까지 Gmail 동기화가 30초를 넘겨 실패한다** |
...
| 8테이블 | 인덱스 **8종 추가 + 중복 3종 제거**(목록: [reference/db-schema.md](./reference/db-schema.md) "미반영 대기 중인 제약·인덱스". FK 컬럼의 `idx_subscription_user`·`idx_mail_sync_logs_account` 는 drizzle push 가 생성보다 삭제를 먼저 실행해 `ER_DROP_INDEX_FK` 가 나므로 유지) | 4차 배치(P-02) | 인덱스만 바뀌므로 응답 계약은 불변이다. **적용 후 동점 정렬 순서가 바뀔 수 있다** — `comments`(postId, created_at)·`messages`(userId, deleted_at, created_at) 목록이 filesort 대신 인덱스 순 스캔이 되어 같은 초에 저장된 행의 상대 순서가 달라진다. 코드에는 이미 `commentId` desc·`id` desc 타이브레이크를 넣어 결정적으로 고정했으므로, **인덱스 push 는 4차 코드가 배포된 뒤에** 한다 |
