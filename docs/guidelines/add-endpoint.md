...
- **[../reference/api-endpoints.md](../reference/api-endpoints.md)**: 해당 도메인 표에 행(`Method | 전체 Path | 인증 | 설명 | 핸들러 파일`) 추가, 그 도메인의 "파일 카운트" 문장과 하단 "파일별 라우트 카운트(자기검증)" 표, `API(/api/*) 라우트 등록 합계`(현재 148)를 함께 갱신한다.
