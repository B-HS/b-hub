...
> 기준: 2026-09-07 (fix/audit-batch4-performance @ 4차 배치 반영) 코드 검증. 다루는 코드: `index.ts`, `compose/index.ts`·`compose/types.ts`·`compose/shared.ts`·`compose/mail.ts`·`compose/ai.ts`, `route/index.ts`, `middleware/*`, `lib/`(error-code·error-message·error·api-response·with-auth·with-error-handling·rate-limit·with-rate-limit·hono-types·env·log-service-name·sentry·cron-auth), `service/shared/redis-client.ts`·`rate-limit-store.ts`·`redis-cache.ts`, `db/index.ts`, `page/index.ts`·`page/admin/guard.ts`, `tsconfig.json`·`vercel.json`·`api/index.js`·`package.json`·`bunfig.toml`·`drizzle.config.ts`
...
- **목록 ServiceDb 는 count/select 를 병렬로 돌린다**(4차 P-01): 페이지네이션 목록 구현은 데이터 쿼리와 `COUNT(*)` 쿼리를 각각 만든 뒤 `const [data, [{ count }]] = await Promise.all([listQuery, countQuery])` 로 **한 번에 await** 한다. blog·drive·ai·logs·metrics 와 어드민 읽기 계층(`page/admin/db.ts` 의 목록 조회 전반과 대시보드 `counts()`)이 이 형태이며, 신규 목록 구현도 직렬 await 를 만들지 않는다. 반환 형태(`{ data, total }`)와 응답은 불변이다.
- **`composeShared.getGdriveAccessToken` 은 프로세스 내 토큰 캐시를 갖는다**(4차 P-05): Google 토큰 교환 응답의 `expires_in` 에서 만료 60초 전까지를 유효기간으로 잡아 클로저에 보관하고, 유효하면 refresh token 조회·교환을 건너뛴다. 자세한 계약은 [reference/shared-services.md](./reference/shared-services.md).
